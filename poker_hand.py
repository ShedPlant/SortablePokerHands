...
class PokerHand(object):
    ...
    def __init__(self, hand_as_string):
        ...
        hand_sorted_by_group_value = collections.Counter(getattr(card, 'value') for card in hand_as_list_of_card)
        #self._logger.debug(hand_sorted_by_group_value)
        #Counter({<CardValue.Nine: '9'>: 2, <CardValue.Five: '5'>: 1, <CardValue.King: 'K'>: 1, <CardValue.Three: '3'>: 1})

        hand_sorted_by_group_value = dict(sorted(hand_sorted_by_group_value.items(), key=lambda item: item[0]))
        #self._logger.debug(hand_sorted_by_group_value)
        # {<CardValue.King: 'K'>: 1, <CardValue.Nine: '9'>: 2, <CardValue.Five: '5'>: 1, <CardValue.Three: '3'>: 1}

        hand_sorted_by_group_value = dict(sorted(hand_sorted_by_group_value.items(), key=lambda item: item[1], reverse=True))
        #self._logger.debug(hand_sorted_by_group_value)
        # {<CardValue.Nine: '9'>: 2, <CardValue.King: 'K'>: 1, <CardValue.Five: '5'>: 1, <CardValue.Three: '3'>: 1}

        group_counts = list(hand_sorted_by_group_value.values())
        first_group_count = group_counts[0]
        second_group_count = group_counts[1]
        if first_group_count == 1:
            if (CardValue.Ace   in hand_sorted_by_group_value and
                CardValue.Two   in hand_sorted_by_group_value and
                CardValue.Three in hand_sorted_by_group_value and
                CardValue.Four  in hand_sorted_by_group_value and
                CardValue.Five  in hand_sorted_by_group_value):
                # Move Ace to Ace Low
                # This hand combination is the *only* time it's ever used
                hand_sorted_by_group_value[CardValue.AceLow] = hand_sorted_by_group_value.pop(CardValue.Ace)
                
            previous_val = None
            straight = True
            for current_val in hand_sorted_by_group_value:
                if previous_val:
        ...
        self.hand_sorted_for_draw_comparison = list(hand_sorted_by_group_value.keys())
